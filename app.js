...
// NEW: view report button (now generates report first)
...
// ----- Report helpers (NEW) -----
async function generateReportAndOpen({ newTab } = { newTab: false }) {
  stopSpeaking();
...
btnStart.addEventListener("click", async () => {
  ...
  // darken after scenario begins
  document.body.classList.add("scenario-active");

  btnEnd.disabled = false;
  if (btnViewReport) btnViewReport.disabled = false;

  // enable talking after start
  callActive = true;
  btnTalk.disabled = false;
...
btnEnd.addEventListener("click", async () => {
  ...
  await generateReportAndOpen({ newTab: false });
});

// NEW: View report should generate report first (otherwise it can be empty)
if (btnViewReport) {
  btnViewReport.addEventListener("click", async (e) => {
    e.preventDefault();
    await generateReportAndOpen({ newTab: true });
  });
}
